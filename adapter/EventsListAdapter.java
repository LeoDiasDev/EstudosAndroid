...
public class EventsListAdapter extends RecyclerView.Adapter<EventsListAdapter.GroupsItemInvitedsViewHolder> {

    Context mContext;
    private List<EventsResultVO.ResultInfo> item;

    public EventsListAdapter(Context context, List<EventsResultVO.ResultInfo> item){
        this.item = item;
        mContext = context;
    }
    ...
    public void onBindViewHolder(GroupsItemInvitedsViewHolder holder, final int position) {
        ...
        holder.distance.setText(item.get(position).distance+"K");
        holder.date.setText(Utils.convertDate2(item.get(position).date));
        holder.name.setText(item.get(position).title);
        holder.address.setText(Utils.capitalize(item.get(position).address_start));

        int color = mContext.getResources().getColor(R.color.blue);
        int colorButton = R.drawable.bg_button_blue;

        if(item.get(position).distance == 10){
            color = mContext.getResources().getColor(R.color.blue);
            colorButton = R.drawable.bg_button_blue;
        }else if(item.get(position).distance == 5){
        ...
        if(item.get(position).team){
            holder.bt_run_info.setText("REVEZAMENTO");
        } else{
            holder.bt_run_info.setText("INDIVIDUAL");
        }

        final EventsResultVO.ResultInfo eventItem = item.get(position);
